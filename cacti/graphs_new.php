			...
			if ($form_type == "cg") {
				...
				while (list($form_id2, $form_array3) = each($form_array2)) {
					...
					$snmp_query = db_fetch_row("select
						snmp_query.name as name,
						snmp_query.xml_path as xml_path
						from snmp_query
...
function graphs() {
	...
	<table width="100%" cellpadding="0" align="center">
		<tr>
			...
			<td width="1">
				...
				<?php
				$hosts = db_fetch_assoc("select id,description,hostname from host order by description,hostname");

				if (sizeof($hosts) > 0) {
					foreach ($hosts as $item) {
						$name=$item["description"]." - (".$item["hostname"].") ";
						print "<option value='" . $item["id"] . "'"; if ($_REQUEST["host_id"] == $item["id"]) { print " selected"; } print ">" . htmlspecialchars($name) . "</option>\n";
					}
				}
				?>
				</select>
	...
	if ($_REQUEST["graph_type"] < 0) {
		...
		$available_graph_templates = db_fetch_assoc("SELECT
			...
			FROM graph_templates LEFT JOIN snmp_query_graph 
	...
	if ($_REQUEST["graph_type"] != -1) {
		...
		foreach ($snmp_queries as $snmp_query) {
			...
			$data_query_graphs = db_fetch_assoc("select snmp_query_graph.id as id,snmp_query_graph.name as name from snmp_query_graph where snmp_query_graph.snmp_query_id=" . $snmp_query["id"] . " order by snmp_query_graph.name");
