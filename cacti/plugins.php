...
function plugins_temp_table_exists($table) {
	return sizeof(db_fetch_row("select * from sqlite_master where tbl_name='$table' and type='table'"));
}
...
function plugins_load_temp_table() {
	...
	while ($x < 30) {
		if (!plugins_temp_table_exists($table)) {
			...
			db_execute("CREATE TEMPORARY TABLE IF NOT EXISTS $table as select * from plugin_config where 1=2");
