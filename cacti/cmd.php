...
function record_cmdphp_done($pid = "") {
	...
	db_execute("UPDATE poller_time SET end_time=datetime() WHERE pid=" . $pid);
}
...
function record_cmdphp_started() {
	...
	db_execute("INSERT INTO poller_time (poller_id, pid, start_time, end_time) VALUES (0, " . getmypid() . ", datetime(), '0000-00-00 00:00:00')");
}
		...
		if (($new_host) && (!empty($host_id))) {
			...
			if (!$host_down) {
				...
				$reindex = db_fetch_assoc("select
					poller_reindex.data_query_id as data_query_id,
					poller_reindex.action as action,
					poller_reindex.op as op,
					poller_reindex.assert_value as assert_value,
					poller_reindex.arg1 as arg1
					from poller_reindex
				...
				if ((sizeof($reindex) > 0) && (!$host_down)) {
					...
					foreach ($reindex as $index_item) {
						...
						if (($index_item["op"] == "=") && ($index_item["assert_value"] != trim($output))) {
							...
							db_execute("replace into poller_command (poller_id, time, action, command) values (0, datetime(), " . POLLER_COMMAND_REINDEX . ", '" . $item["host_id"] . ":" . $index_item["data_query_id"] . "')");
							$assert_fail = true;
