...
if ($action == 'login') {
	...
	if (sizeof($user) > 0) {
		...
		db_execute("INSERT INTO user_log (username,user_id,result,ip,time) VALUES (" . $cnn_id->qstr($username) . "," . $user["id"] . ",1,'" . $_SERVER["REMOTE_ADDR"] . "',datetime())");
		/* is user enabled */
		...
		if ((!$guest_user) && ($user_auth)) {
			...
			db_execute("INSERT INTO user_log (username,user_id,result,ip,time) VALUES (" . $cnn_id->qstr($username) . ",0,0,'" . $_SERVER["REMOTE_ADDR"] . "',datetime())");
		}
	}
}
