...
function item_edit() {
	...
	case TREE_ITEM_TYPE_HOST:
		form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],0); ?>
			...
			<td>
				<?php form_dropdown("host_id", db_fetch_assoc("select id,description||' ('||hostname||')' as name from host order by description,hostname"), "name", "id", (isset($tree_item["host_id"]) ? $tree_item["host_id"] : ""), "", "");?>
			</td>
		</tr>
		<?php form_alternate_row_color($colors["form_alternate1"],$colors["form_alternate2"],1); ?>
			<td width="50%">
...
function item_remove() {
	...
	if ((read_config_option("deletion_verification") == "on") && (!isset($_GET["confirm"]))) {
		...
		if (!empty($graph_tree_item["local_graph_id"])) {
			...
			$text = "Are you sure you want to delete the host item <strong>'" . db_fetch_cell("select description||' ('||hostname||')' as hostname from host where id=" . $graph_tree_item["host_id"]) . "'</strong>?";
		}
