...
function grow_graph_tree($tree_id, $start_branch, $user_id, $options) {
	...
	$hier_sql = "select
		...
		graph_tree_items.rra_id as rra_id,
		...
		host.description||' ('||host.hostname||')' as hostname,
		settings_tree.status as status
...
function grow_edit_graph_tree($tree_id, $user_id, $options) {
	...
	$tree = db_fetch_assoc("select
		graph_tree_items.id as id,
		graph_tree_items.title as title,
		graph_tree_items.graph_tree_id as graph_tree_id,
		graph_tree_items.local_graph_id as local_graph_id,
		graph_tree_items.host_id as host_id,
		graph_tree_items.order_key as order_key,
		graph_tree_items.sort_children_type as sort_children_type,
		graph_templates_graph.title_cache as graph_title as graph_title,
	...
	if (sizeof($tree_list) > 0) {
		foreach ($tree_list as $tree) {
			...
			$hierarchy = db_fetch_assoc("select
				...
				graph_tree_items.order_key as order_key,
				graph_tree_items.host_id as host_id,
				graph_tree_items.host_grouping_type as host_grouping_type,
