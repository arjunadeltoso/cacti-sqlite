...
function update_poller_cache($local_data_id, $commit = false) {
	...
	$data_input = db_fetch_row("select
		...
		data_template_data.data_template_id as data_template_id,
	...
	if (($data_input["type_id"] == DATA_INPUT_TYPE_SNMP_QUERY) ||
		...
		$outputs = db_fetch_assoc("select
			snmp_query_graph_rrd.snmp_field_name as snmp_field_name,
...
function poller_update_poller_cache_from_buffer($local_data_ids, &$poller_items) {
	...
	$sql_prefix   = "INSERT OR IGNORE INTO poller_item (local_data_id, poller_id, host_id, action, hostname, " .
	...
	$sql_suffix=''; # for sqlite

	/* use a reasonable insert buffer, the default is 1MByte */
	#$max_packet   = 256000;
	$max_packet   = 1028; #absurdly short to ensure only 1 record per insert
...
function push_out_host($host_id, $local_data_id = 0, $data_template_id = 0) {
	...
	$data_sources = db_fetch_assoc("SELECT
		...
		data_template_data.local_data_template_data_id as local_data_template_id,
