...
include_once($config["library_path"]."/adodb/adodb-errorhandler.inc.php");
function db_connect_real($host, $user, $pass, $db_name, $db_type, $port = "3306", $db_ssl = false, $retries = 20) {
	global $cnn_id;

	if ($db_type == "sqlite") {
		$path = urlencode($port); # path is specified in database_port;
		$dsn = "pdo_sqlite://$path/?persist,debug";  # persist is optional
		$cnn_id = ADONewConnection($dsn);  # no need for Connect/PConnect
		if ($cnn_id) {
			return($cnn_id);
		} else {
			die("FATAL: Cannot connect to sqlite db at $port\n");
		}
	} else {
	$i = 0;
...
#print "running assoc $sql\n<br>";
	if (($db_conn->ErrorNo() == 0) || ($db_conn->ErrorNo() == 1032) && ($query)) {
