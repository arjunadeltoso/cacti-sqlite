...
function move_branch($dir, $order_key, $table, $field, $where) {
	...
	if (sizeof($displaced_row) > 0) {
		...
		db_execute("UPDATE $table SET $field = ('" . str_pad('', ($tier * CHARS_PER_TIER), 'Z') . "',SUBSTRING($field," . (($tier * CHARS_PER_TIER) + 1).")) WHERE $field LIKE '$new_root%'$where");
		#db_execute("UPDATE $table SET $field = CONCAT('$new_root',SUBSTRING($field," . (($tier * CHARS_PER_TIER) + 1) . ")) WHERE $field LIKE '$old_root%' $where");
		db_execute("UPDATE $table SET $field = '$new_root'||SUBSTRING($field," . (($tier * CHARS_PER_TIER) + 1) . ") WHERE $field LIKE '$old_root%' $where");
		...
		db_execute("UPDATE $table SET $field = '$old_root'||SUBSTRING($field," . (($tier * CHARS_PER_TIER) + 1) . ") WHERE $field LIKE '".str_pad('', ($tier * CHARS_PER_TIER), 'Z') . "%' $where");
	}
...
function sort_tree($sort_type, $item_id, $sort_style) {
	...
	while (list($_tier_key, $tier_array) = each($leaf_sort_array)) {
		while (list($_search_key, $search_array) = each($tier_array)) {
			...
			db_execute("update graph_tree_items set order_key = 'x'||order_key where order_key like '$_search_key%%' " . (($sort_type == SORT_TYPE_TREE_ITEM) ? "and id != $item_id" : "and order_key != '$_search_key" . str_repeat('0', (MAX_TREE_DEPTH * CHARS_PER_TIER) - strlen($_search_key)) . "'") . " and graph_tree_id = '$tree_id'");
...
function reparent_branch($new_parent_id, $tree_item_id) {
	...
	db_execute("update graph_tree_items set order_key = 'x'||order_key where order_key like '$old_base_tier%%' and graph_tree_id=$graph_tree_id");
	...
	/* append */
		...
		db_execute("update graph_tree_items set order_key = REPLACE(order_key, 'x$old_base_tier', '$new_base_tier')|| '" . str_repeat('0', (strlen($old_base_tier) - strlen($new_base_tier))) . "' where order_key like 'x$old_base_tier%%' and graph_tree_id=$graph_tree_id");
	}
}
