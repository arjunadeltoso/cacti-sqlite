...
function run_data_query($host_id, $snmp_query_id) {
	...
	$type_id = db_fetch_cell("select data_input.type_id as type_id from (snmp_query,data_input) where snmp_query.data_input_id=data_input.id and snmp_query.id=$snmp_query_id");
...
function data_query_update_host_cache_from_buffer($host_id, $snmp_query_id, &$output_array) {
	...
	$sql_prefix   = "INSERT or ignore INTO host_snmp_cache (host_id, snmp_query_id, field_name, field_value, snmp_index, oid, present) VALUES";
...
function data_query_index($index_type, $index_value, $host_id, $data_query_id) {
	return db_fetch_cell("select
		host_snmp_cache.snmp_index as snmp_index
...
function get_formatted_data_query_indexes($host_id, $data_query_id) {
	...
	$sort_field_data = array_rekey(db_fetch_assoc("select
		...
		host_snmp_cache.field_value as field_value
