...
function get_full_script_path($local_data_id) {
	...
	$data_source = db_fetch_row("select
		data_template_data.id as id,
		data_template_data.data_input_id as data_input_id,
		data_input.type_id as type_id,
		data_input.input_string as input_string
...
function get_data_source_item_name($data_template_rrd_id) {
	...
	$data_source = db_fetch_row("select
		...
		data_template_data.name as name
...
function get_data_source_title($local_data_id) {
	$data = db_fetch_row("select
		...
		data_local.snmp_query_id as snmp_query_id,
		data_local.snmp_index as snmp_index,
		data_template_data.name as name
...
function get_graph_title($local_graph_id) {
	$graph = db_fetch_row("select
		...
		graph_templates_graph.title as title
	...
	if (!isset($_SESSION["tree_array"]) || ($force_refresh) ||
		...
		if (read_config_option("auth_method") != 0) {
			...
			$sql = "select
				...
				user_auth_perms.user_id as user_id
				from graph_tree
...
function get_host_array() {
	if (read_config_option("auth_method") != 0) {
		...
		$host_list = db_fetch_assoc("select
			host.id as id,
			host.description||' ('||host.hostname||')' as name,
			user_auth_perms.user_id as user_id
			from host
		...
		$host_list = db_fetch_assoc("select id,description||' ('||hostname||')' as name from host order by description,hostname");
	}
