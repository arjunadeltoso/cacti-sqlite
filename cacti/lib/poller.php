...
function update_reindex_cache($host_id, $data_query_id) {
	...
	$data_query_type = db_fetch_cell("select data_input.type_id as type_id from (data_input,snmp_query) where data_input.id=snmp_query.data_input_id and snmp_query.id=$data_query_id");
...
function process_poller_output(&$rrdtool_pipe, $remainder = FALSE) {
	...
	$results = db_fetch_assoc("select
		poller_output.output as output,
		poller_output.time as time,
		poller_output.time as time,
		poller_output.local_data_id as local_data_id,
