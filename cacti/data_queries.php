...
function form_actions() {
	...
	while (list($var,$val) = each($_POST)) {
		if (preg_match("/^chk_([0-9]+)$/", $var, $matches)) {
			...
			$dq_list .= "<li>" . htmlspecialchars(db_fetch_cell("SELECT snmp_query.name as name FROM snmp_query WHERE id='" . $matches[1] . "'")) . "<br>";
		...
		if (sizeof($data_templates) > 0) {
			foreach ($data_templates as $data_template) {
				...
				$data_template_rrds = db_fetch_assoc("select
					...
					snmp_query_graph_rrd.snmp_query_graph_id as snmp_query_graph_id
