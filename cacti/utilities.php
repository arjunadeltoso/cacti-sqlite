...
function utilities_view_tech($php_info = "") {
	...
	#$tables = db_fetch_assoc("select tbl_name as Tables_in_main from sqlite_master where type='table'");
	$tables = db_fetch_assoc("select tbl_name as blah from main.sqlite_master ");
	$skip_tables  = array();
	$table_status = array();
	...
	foreach($tables as $table) {
		...
		$create_syntax = db_fetch_row("select sql from sqlite_master where tbl_name='" . $table["blah"]."'");
		#print $table["blah"]."\n<br>";

		if (sizeof($create_syntax)) {
			#if (substr_count(strtoupper($create_syntax["Create Table"]), "INNODB")) {
			if (substr_count(strtoupper($create_syntax["sql"]), "INNODB")) {
				$skip_tables[] = $table["blah"];
			}else{
				$include_tables[] = $table["blah"];
			}
		}
	}
	}
	...
	foreach($include_tables as $table) {
		...
		$status = db_fetch_row("select '$table' as Name, count(*) as Rows from '$table'");
...
function utilities_view_poller_cache() {
	...
	$total_rows = db_fetch_cell("SELECT
		COUNT(*)
		FROM poller_item  LEFT JOIN host  ON poller_item.host_id=host.id
		LEFT JOIN data_template_data ON data_template_data.local_data_id=poller_item.local_data_id 
		$sql_where");

	$poller_sql = "SELECT
		poller_item.local_data_id as local_data_id,    
		poller_item.poller_id as poller_id,
		poller_item.host_id as host_id,
		poller_item.action as action,
		poller_item.present as present,
		poller_item.hostname as hostname,
		poller_item.snmp_community as snmp_community,
		poller_item.snmp_version as snmp_version,
		poller_item.snmp_username as snmp_username,
		poller_item.snmp_password as snmp_password,
		poller_item.snmp_auth_protocol as snmp_auth_protocol,
		poller_item.snmp_priv_passphrase as snmp_priv_passphrase,
		poller_item.snmp_priv_protocol as snmp_priv_protocol,
		poller_item.snmp_context as snmp_context,
		poller_item.snmp_port as snmp_port,
		poller_item.snmp_timeout as snmp_timeout,
		poller_item.rrd_name as rrd_name,
		poller_item.rrd_path as rrd_path,
		poller_item.rrd_num as rrd_num,
		poller_item.rrd_step as rrd_step,
		poller_item.rrd_next_step as rrd_next_step,
		poller_item.arg1 as arg1,
		poller_item.arg2 as arg2,
		poller_item.arg3 as arg3,  
		data_template_data.name_cache as name_cache,  
