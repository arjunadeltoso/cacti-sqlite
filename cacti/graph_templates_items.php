		...
		foreach ($items as $item) {
			...
			if (!is_error_message()) {
				...
				$orig_data_source_graph_inputs = db_fetch_assoc("select
					graph_template_input.id as id,
					graph_template_input.name as name,
					graph_templates_item.task_item_id as task_item_id
...
function item_edit() {
	...
	$struct_graph_item["task_item_id"]["sql"] = "select
		data_template.name||' - '||' ('||data_template_rrd.data_source_name||')' as name,
