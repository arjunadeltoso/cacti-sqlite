		...
		if ($_POST["drp_action"] == "1") { /* delete */
			...
			if (isset($graph_array) && sizeof($graph_array)) {
				$data_sources = db_fetch_assoc("select
					data_template_data.local_data_id as local_data_id,
			...
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						...
						<p><ul>$graph_list</ul></p>
						<p><strong>New Graph Template:</strong><br>"; form_dropdown("graph_template_id",db_fetch_assoc("select graph_templates.id as id,graph_templates.name as name from graph_templates order by name"),"name","id","","","0"); print "</p>
					</td>
				</tr>\n
				";
...
function graph_diff() {
	...
	$template_query = "select
		...
		graph_templates_item.consolidation_function_id as consolidation_function_id,
		graph_templates_item.graph_type_id as graph_type_id,
		data_template_data.name as concat_name,
		data_template_rrd.data_source_name as concat_data_source_name
	...
	$graph_template_inputs = db_fetch_assoc("select
		graph_template_input.column_name as column_name,
		graph_template_input_defs.graph_template_item_id as graph_template_item_id
		from (graph_template_input,graph_template_input_defs)
		where graph_template_input.id=graph_template_input_defs.graph_template_input_id
		and graph_template_input.graph_template_id=" . $_GET["graph_template_id"]);

	# fix task_item_id;
	$task_item_id=$concat_name." - ".$concat_data_source_name;
	$item["task_item_id"]=$task_item_id;
...
function graph_edit() {
	...
	$form_array = array(
		"graph_template_id" => array(
			...
			"none_value" => "None",
			"sql" => "select graph_templates.id as id,graph_templates.name as name from graph_templates order by name"
			),
		"host_id" => array(
			"method" => "drop_sql",
			"friendly_name" => "Host",
					...
					<td width="1">
						<select name="host_id" onChange="applyGraphsFilterChange(document.form_graph_id)">
							...
							if (read_config_option("auth_method") != 0) {
								...
								$hosts = db_fetch_assoc("SELECT DISTINCT host.id as id, host.description||' ('||host.hostname||')' as name
								...
								$hosts = db_fetch_assoc("SELECT DISTINCT host.id, host.description||' ('||host.hostname,')' as name
									FROM host
									ORDER BY name");
							}
