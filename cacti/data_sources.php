...
function form_save() {
	...
	if ((isset($_POST["save_component_data"])) && (!is_error_message())) {
		...
		$input_fields = db_fetch_assoc("select
			data_template_data.data_input_id as data_input_id,
			data_local.host_id as host_id,
			data_input_fields.id as id,
			data_input_fields.input_output as input_output,
			data_input_fields.data_name as data_name,
			data_input_fields.regexp_match as regexp_match,
			data_input_fields.allow_nulls as allow_nulls,
		...
		if ($_POST["drp_action"] == "1") { /* delete */
			...
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						...
						<p><ul>$ds_list</ul></p>
						<p><strong>New Data Template:</strong><br>"; form_dropdown("data_template_id",db_fetch_assoc("select data_template.id as id,data_template.name as name from data_template order by data_template.name"),"name","id","","","0"); print "</p>
					</td>
				</tr>\n
				";
			...
			print "	<tr>
					<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
						...
						<p><ul>$ds_list</ul></p>
						<p><strong>New Host:</strong><br>"; form_dropdown("host_id",db_fetch_assoc("select id,description||' ('||hostname||')' as name from host order by description,hostname"),"name","id","","","0"); print "</p>
					</td>
				</tr>\n
				";
...
function data_edit() {
	...
	if (!empty($_GET["id"])) {
		...
		$host = db_fetch_row("select host.id as id,host.hostname as hostname from (data_local,host) where data_local.host_id=host.id and data_local.id=" . $_GET["id"]);
...
function ds_edit() {
	...
	$form_array = array(
		...
		"host_id" => array(
			...
			"none_value" => "None",
			"sql" => "select id,description||' ('||hostname||')' as name from host order by description,hostname"
			),
	...
	if (((isset($_GET["id"])) || (isset($_GET["new"]))) && (empty($data["data_template_id"]))) {
		...
		if ((empty($data["data_input_id"])) || (db_fetch_cell("select type_id from data_input where id=" . $data["data_input_id"]) > "1")) {
			...
			$struct_data_source_item["data_input_field_id"]["sql"] = "select id,data_name||' - '||name as name from data_input_fields where data_input_id=" . $data["data_input_id"] . " and input_output='out' and update_rra='on' order by data_name,name";
		}
...
function ds() {
				...
				<tr>
					...
					<td>
						<select name="host_id" onChange="applyDSFilterChange(document.form_data_sources)">
							...
							<?php
							$hosts = db_fetch_assoc("select id,description||' ('||hostname||')' as name from host order by description,hostname");
	...
	$poller_intervals = array_rekey(db_fetch_assoc("SELECT data_template_data.local_data_id AS id, 
		...
		FROM data_template, data_local, data_template_data_rra, data_template_data, rra
		WHERE data_template_data_rra.data_template_data_id=data_template_data.id
